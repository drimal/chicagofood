...
@click.command()
@click.option('--startdate', default='2017-01-01', help='start date')
...
def download_food_inspections(client, startdate):
    ...
    results_df.to_json('../../data/raw/food-inspection.json')
...
def download_crime_data(client, startdate):
    ...
    # Convert to pandas DataFrame
    results_df = pd.DataFrame.from_records(results)
    results_df.to_json('../../data/raw/burglary.json')
    #client.close()
...
def download_service_request_data_after_july01_2018(client):
    ...
    # Convert to pandas DataFrame
    results_df = pd.DataFrame.from_records(results)
    results_df.to_json('../../data/raw/servicedata.json')
...
def download_service_request_data_before_july01_2018(client, startdate):
    ...
    for i in range(len(dataset_identifiers)):
        ...
        # Convert to pandas DataFrame
        results_df = pd.DataFrame.from_records(results)
        outfile = "../../data/raw/%sdata_prior_to_july2018.json" %dataset_names[i]
...
def download_business_data(client, startdate):
    ...
    # Convert to pandas DataFrame
    results_df = pd.DataFrame.from_records(results)
    results_df.to_json('../../data/raw/business-license.json')
